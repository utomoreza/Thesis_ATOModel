...
max_speed = 22.2;
max_brake = 260000; %310000; % use this specially for PID braking test %257880;
...
% if you wanna test the system for BRAKING FORCE TEST with initial speed
% of 20 m/s until distance of 350 m then applying braking until speed of
% 5 m/s, uncomment lines below:
v_ref(1:350,1) = 20;
v_ref(351:end,1) = 5;
v_ref_ori = v_ref;
v_ref2 = v_ref;
...
% v_lim(:,1) = max_speed;
v_ref = zeros(size(s));
% For Su(2016) model, use v_ref = 1;
% For DLR model, use v_ref = 3;
v_ref(:,1) = 1; %3;
relay_amp = 1;
for i = 1:S_max
...
%     % if you wanna test the system without the controller,
%     % uncomment lines below and comment pid_control.
%     % *****************************************************
%     u(i,1) = v_ref2(i+1,1) - v(i,1);    
%     u_nolimit(i,1) = u(i,1);
%     if u(i,1) > 1
%         u(i,1) = 1;
%     end
...
% IF YOU WANNA USE NORMAL FUZZY GAIN SCHEDULING, UNCOMMENT 1st OPTION;
% IF YOU WANNA USE TRAJECTORY SWITCH APPROACH, UNCOMMENT 2nd OPTION;
    normalRunning; %1st OPTION
%     System_TrajecSwitch; %2nd OPTION
