...
% v_ref2 = vel';
% fuz_sch1 = readfis('fuzzy_scheduling2_resist');
% fuz_sch = mam2sug(fuz_sch1);
...
% FUZZY GAIN SHCEDULING -- MORE COMPLEX
% fuzz_in = zeros(length(s),6);
% fuzz_out = zeros(length(s),3);
% ******************************************
...
% % ********************************************************************
% % ZIEGLER-NICHOLS TUNING -- FOR ICIRT PAPER ONLY
% v_ref = zeros(size(s));
% v_ref(:,1) = 1;
% kcr = 1.837;
% for i = 1:S_max
...
% [pks,idx] = findpeaks(v);
% highest_peak = find(pks==max(pks));
% highest_peak_idx = idx(highest_peak);
% period1 = Time(highest_peak_idx(1,1)) - Time(highest_peak_idx(1,1)-1);
% period2 = Time(highest_peak_idx(1,1)+1) - Time(highest_peak_idx(1,1));
% pcr = (period1 + period2)/2;
% v = zeros(size(s));
% Time = zeros(size(s));
% del_T = zeros(size(s));
% grad_res = grad(:,1)/10;
...
coasting = 0; %0 for no-coasting mode; 1 for coasting mode
coasting_vel = 60;
for i = 1:del_S:S_max
    traction;
...
load v_ref; v_ref2 = v_ref; %v_ref2 = v;
v(:,1) = 0;
error(:,1) = 0;
u(:,1) = 0;
acceleration(:,1) = 0;
...
for j = 1:del_S:S_max
    ...
    % calculate running_time (Tr) and running_time_ref (Tr_ref) every 1 metre
    % compare Tr and Tr_ref
    % assign the results as time difference in per cent
%     if j == 1
%         for i = j:del_S:S_max
...
%             jerk(i,1) = (accel(i+1,1) - accel(i,1))/del_T(i+1,1);
%         end
%         
%         maxTime = Time(end,1);
%         maxTime_ref = Time_ref(end,1);
%         TrMinTref(j:j+99,1) = (maxTime - maxTime_ref)/100;
%     end
%     
%     if j == step(j,1)
%         vel(1:j,1) = v(1:j,1);
...
% %     Time_ref(i+1,1)= Time_ref(i,1)+del_T_ref(i+1,1);
% else
%     del_T(i,1) = 2*del_S/(v(i,1)+v(i-1,1));
% %     Time(i+1,1)= Time(i,1)+del_T(i+1,1);
%     del_T_ref(i,1) = 2*del_S/(v_ref_ori(i,1)+v_ref_ori(i-1,1));
% %     Time_ref(i+1,1)= Time_ref(i,1)+del_T_ref(i+1,1);
% end
%     TrMinTref(i,1) = (del_T(i,1) - del_T_ref(i,1))/100;  
...
%     % if you wan to test the system without the controller,
...
%     fuzz_in(i,1) = v(i,1);
%     fuzz_in(i,2) = 0;
%     fuzz_in(i,3) = error(i,1);
%     fuzz_in(i,4) = del_e(i,1);
%     fuzz_in(i,5) = TrMinTref(i,1);
%     fuzz_in(i,6) = accRtot(i,1);
%     fuzz_out(i,:) = evalfis(fuzz_in(i,:),fuz_sch);
%     kp = fuzz_out(i,1);
%     ki = fuzz_out(i,2);
%     kd = fuzz_out(i,3);
    % ***********************************
...
%     end
% ********************************************************************
% if error(i,1) >= del_one;
%     u(i,1) = relay_amp;
% elseif error(i,1) <= -del_two;
%     u(i,1) = -relay_amp;
% else
%     u(i,1) = u(i-1,1);
% end
...
% u_dft = fft(error);
% [maxval,idx] = max(abs(u_dft));
% freq = (Fs*(idx-1))/(length(u));
% period = 1/freq
% % Find amplitude
% error_peaks = findpeaks(error);
% avg_error_peak = max(error_peaks)
% max_peaks = findpeaks(v);
% min_peaks = findpeaks(-v);
% avg_max_peaks = mean(max_peaks);
% avg_min_peaks = mean(min_peaks);
% amp = (avg_max_peaks - (-avg_min_peaks))/2
% ********************************************************************
...
dev = v_ref2 - v;
e_percent_pos = v_ref2./100*10;
over_pos = v_ref2./100*10;
...
legend('error deviation','+5% error limit','-5% error limit')
axis tight
...
% PROGRESS
% USE COASTING TRAJECTORY AS REFERENCE !!!
% THERE'S SOMETHING WRONG ON KALMAN FILTER
    % IT SEEMS IT'S THE MODEL WHICH MAKES THE PROBLEM
    % OR MAYBE IT'S DUE TO ONLY-LINEAR USE OF KALMAN FILTER
    % NEED TO IMPROVE THE FILTER
